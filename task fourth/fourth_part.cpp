...
void set_array_H (double *H, double time, double h, int M) {
	for (int i = 0; i < M; i++) { H[i] = r(time, (i + 0.5) * h); }
}

void set_array_V (double *V, double time, double h, int M) {
	for (int i = 0; i <= M; i++) { V[i] = u(time, i * h); }
}
...
void print_H_like_gnu(double *z, double time_x, double h, int M) {
	int j = 0;
...
int set_matrix_H (double *H_under, double *H_main, double *H_above, double *V, double *right_part, double *H_prev, double tau, double h, int timestep, int M) {
	H_above[0] = 0.5 * (V[1] - fabs(V[1])) / h * tau;
	H_main[0]  = 1 + 0.5 * (V[1] + fabs(V[1])) / h * tau;
	H_under[0] = 0;
	right_part[0] = /* H_prev[0] + */ rho_tilde;
	//right_part[0] = H_prev[0] + f_1((timestep) * tau, 0.5 * h) * tau;		// вот это вот было раньше
...
int set_matrix_V (double *V_under, double *V_main, double *V_above, double *H, double *right_part, double *H_prev, double *V_prev, double tau, double h, int M, int time_step, mode_my_mode rezhym) {
	V_above[0] = 0;
	V_main[0] = 1;
	V_under[0] = 0;
	right_part[0] = /* V_prev[0] + */ v_tilde;
	...
	right_part[M] = 0;
	//right_part[0] = v_tilde;		// мб вот эта штука не работает
	...
	//right_part[0] = v_tilde;
	//right_part[M] = right_part[M-1];					///////// похоже эта строка неверна ////////// ВОТ КАКУЮ СТРОКУ Я ДОБАВИЛ, ЧТОБЫ ПО-НОМРАЛЬНОМУ СЧИТАТЬ ЧЕТВЕРТОЕ ЗАДАНИЕ
...
void residuals(double (*rho_func)(double, double), double *H, double time, double h, int M, double &r1, double &r2, double &r3, double &r4) {
	r1 = fabs(H[0] - rho_func(time, 0.5 * h));
	double buf;
	for (int i = 1; i < M; i++) {
		buf = H[i] - rho_func(time, (i + 0.5) * h);
		r1 = fmax(r1, fabs(buf));
		r2 += buf * buf;
...
int main(int argc, char *argv[]) {
	...
	if (!(argc == 9 
		...
		&& sscanf(argv[4], "%lf",  &v_tilde) == 1
		&& sscanf(argv[5], "%lf",  &rho_tilde) == 1
		&& sscanf(argv[6], "%d",  &C) == 1
		&& sscanf(argv[7], "%lf", &mu) == 1
		&& sscanf(argv[8], "%lf", &time) == 1
		&& fabs(rezhim - 0.5) < 0.6)){ 
			printf("Для откладки\n");
			return -1; }
	...
	set_array_H(H_prev, 0, h, M);
	set_array_V(V_prev, 0, h, M + 1);
	...
	if (kakaya_norma < 0) {
		...
		for (j = 0; j < 10; j++) {
			set_matrix_H(H_under, H_main, H_above, V, H, H_prev, tau, h, j, M);
			...
			set_matrix_V(V_under, V_main, V_above, H, V, H_prev, V_prev, tau, h, M, j, rezh);
		...
		do {			// хз почему надо сделать как минимум два шага, так как после первого она нулевая, соответственно надо по-нормальному
			set_matrix_H(H_under, H_main, H_above, V, H, H_prev, tau, h, j, M);
			// раньше вместо 1 вычитался H_prev[i]
			...
			//for (int i = 1; i < M; i += 1) { buf = V[i] - V_prev[i];		r1 = fmax(r1, fabs(buf)); }		// не смотрим на скорость в граничных точках

			memcpy(H_prev, H, M * sizeof(double)); memcpy(V_prev, V, (M + 1) * sizeof(double));

			if (r1 > 3 /* || r2 / r1 > 1.1 */) {
				printf("$ %cinfty $", '\\');
				return 3;		// нормально завершение, просто не сходится схемка
		...
		for (int i = 0; i < N; i++) {
			...
			int result = set_matrix_H(H_under, H_main, H_above, V_prev, H, H_prev, tau, h, i, M);
			if (result != 0) {
				printf("Плохое решение плотности на шаге %d\n", i);
				return -1;
			}
			//set_array_V(V, i * tau, h, M + 1, mode);
			result = set_matrix_V(V_under, V_main, V_above, H, V, H_prev, V_prev, tau, h, M, i, rezh);
			if (result != 0) {
		...
		residuals(r, H, N * tau, h, M, res1, res2, res3, res4);
		...
		for (int i = 0; i < N + 1; i++) {
			set_matrix_H(H_under, H_main, H_above, V, H, H_prev, tau, h, i, M);
			/*
			if (i % (N / 400) == 0) {
			...
			set_matrix_V(V_under, V_main, V_above, H, V, H_prev, V_prev, tau, h, M, i, rezh);
			memcpy(H_prev, H, M * sizeof(double)); memcpy(V_prev, V, (M + 1) * sizeof(double));
		}
		...
		for (int i = 0; i < N; i++) {
			set_matrix_H(H_under, H_main, H_above, V, H, H_prev, tau, h, i, M);
			if (i % (N / 800) == 0) print_H_like_gnu(H, (i + 1) * tau, h, M);
			set_matrix_V(V_under, V_main, V_above, H, V, H_prev, V_prev, tau, h, M, i, rezh);
			memcpy(H_prev, H, M * sizeof(double)); memcpy(V_prev, V, (M + 1) * sizeof(double));
		}
